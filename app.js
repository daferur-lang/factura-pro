...
function getCounters() {
  return load(S.COUNTERS, { p:0, f:0 });
}
...
function navigate(view, opts={}) {
  ...
  if(view === 'home') {
    ...
    hdrSettings.style.display = 'flex';
...
function renderDocList() {
  ...
  document.querySelectorAll('.tab').forEach(t =>
    t.classList.toggle('active', t.dataset.t === state.tab)
  );
  ...
  docList.innerHTML = sorted.map(d => `
    <div class="doc-card" data-id="${d.id}">
      <div class="doc-card-top">
        <div>
          <div class="doc-card-cliente">${esc(d.cliente.nombre || '—')}</div>
          <div class="doc-card-num">${d.numero}</div>
        </div>
        <span class="badge ${ESTADOS[d.estado]?.cls || ''}">${ESTADOS[d.estado]?.label || d.estado}</span>
      </div>
      <div class="doc-card-bottom">
        <span class="doc-card-total">${fmt(d.total)}</span>
        <span class="doc-card-date">${fmtDate(d.fecha)}</span>
      </div>
    </div>
  `).join('');
...
function renderForm(editId) {
  ...
  document.querySelectorAll('.iva-btn').forEach(b => {
    b.classList.toggle('active', Number(b.dataset.v) === state.iva);
  });
...
function renderLines() {
  ...
  wrap.innerHTML = lines.map((l,i) => `
    <div class="line-item" data-lid="${l.id}">
      <div class="line-top">
        <div class="line-desc">
          <input class="inp" type="text" placeholder="Descripción *" value="${esc(l.desc)}" data-field="desc" data-i="${i}">
        </div>
        ${lines.length > 1 ? `<button type="button" class="line-del" data-del="${i}" aria-label="Eliminar">
      ...
      <div class="line-bottom">
        <div class="line-qty">
          ...
          <input class="inp" type="number" min="0" step="any" value="${l.qty}" data-field="qty" data-i="${i}">
        </div>
        <div class="line-price">
          <label class="lbl">Precio (€)</label>
          <input class="inp" type="number" min="0" step="any" value="${l.price}" data-field="price" data-i="${i}">
        </div>
  ...
  wrap.querySelectorAll('input[data-field]').forEach(inp => {
    inp.addEventListener('input', () => {
      ...
      if(f==='desc') lines[i].desc = inp.value;
      else lines[i][f] = parseFloat(inp.value)||0;
      updateTotals();
...
function calcTotals() {
  const sub = lines.reduce((s,l) => s + l.qty*l.price, 0);
  const ivaAmt = sub * state.iva / 100;
  return { subtotal:sub, ivaAmt, total:sub+ivaAmt };
}
...
function renderDetail(id) {
  ...
  body.innerHTML = `
    <div class="detail-header">
      ...
      <div class="detail-dates">
        <span>Emitido: ${fmtDate(doc.fecha)}</span>
        ${doc.vencimiento?`<span>Vence: ${fmtDate(doc.vencimiento)}</span>`:''}
      </div>
    </div>
    ...
    <div class="detail-section">
      ...
      ${doc.lineas.map(l => `
        <div class="detail-line">
          <div>
            <div class="detail-line-desc">${esc(l.desc||'Sin descripción')}</div>
            <div class="detail-line-meta">${l.qty} × ${fmt(l.price)}</div>
          </div>
          <div class="detail-line-total">${fmt(l.qty*l.price)}</div>
        </div>
      `).join('')}
    </div>
  ...
  const isPresupuesto = doc.tipo === 'presupuesto';
  bar.innerHTML = `
    ${isPresupuesto ? `<button class="btn-primary" id="aConvert">→ Factura</button>` : ''}
    <button class="btn-primary" id="aPdf">PDF</button>
    <button class="btn-ghost" id="aMenu">…</button>
  `;
...
function showDocMenu(id) {
  ...
  const estados = Object.entries(ESTADOS)
    .filter(([k]) => doc.tipo==='factura' || !['pagado'].includes(k));

  opts.innerHTML = estados.map(([k,v]) => `
    <button class="status-opt" data-s="${k}">
      <span class="badge ${v.cls}">${v.label}</span>
    </button>
  `).join('') + `
    <button class="status-opt" data-s="edit" style="color:var(--p)">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
      Editar
    </button>
    <button class="status-opt" data-s="delete" style="color:var(--danger)">
  ...
  opts.querySelectorAll('[data-s]').forEach(btn => {
    btn.addEventListener('click', () => {
...
function convertToInvoice(id) {
  ...
  docs.push(inv);
  saveDocs(docs);
  showToast('Convertido a factura ✓');
  navigate('detail', { id: inv.id });
}
...
function renderSettings() {
  ...
  planCard.innerHTML = `
    ...
    <div class="plan-info">
      <span class="plan-name">${pro?'Pro':'Gratuito'}</span>
      <span class="plan-badge ${pro?'pro':'free'}">${pro?'PRO':'FREE'}</span>
    </div>
    ${!pro ? `
      <div class="plan-meter"><div class="plan-meter-fill" style="width:${pct}%"></div></div>
      <div class="plan-meter-txt">${used} de ${FREE_LIMIT} documentos este mes</div>
      <button class="btn-primary w100" id="upgradeFromSettings">Activar Pro — 4,99 €/mes</button>
    ` : `<p style="font-size:.88rem;color:var(--gray-3)">Acceso ilimitado activado. Gracias ❤️</p>`}
  `;
...
function generatePDF(doc) {
  ...
  const W=210, mL=18, mR=18, col2=W-mR;
  let y = 20;
  const pro = isPro();
  ...
  if(!pro) {
    ...
    pdf.text('DEMO', W/2, 148, { align:'center', angle:45 });
  ...
  pdf.text(p.nombre||'Mi Empresa', mL, 15);
  pdf.setFontSize(9);
  ...
  pdf.setFontSize(20);
  pdf.setTextColor(255,255,255);
  const typeLabel = doc.tipo==='factura' ? 'FACTURA' : 'PRESUPUESTO';
  pdf.text(typeLabel, col2, 14, { align:'right' });
  pdf.setFontSize(11);
  pdf.setFont('helvetica','normal');
  pdf.text(doc.numero, col2, 22, { align:'right' });
  pdf.setFontSize(9);
  ...
  pdf.text('FACTURAR A', mL, y);
  y += 5;
  pdf.setTextColor(0,0,0);
  pdf.setFontSize(11);
  pdf.text(doc.cliente.nombre||'', mL, y);
  y += 5;
  pdf.setFont('helvetica','normal');
  pdf.setFontSize(9);
  ...
  pdf.rect(mL, y, W-mL-mR, 8, 'F');
  pdf.setFont('helvetica','bold');
  pdf.setFontSize(9);
  pdf.setTextColor(107,114,128);
  pdf.text('DESCRIPCIÓN', mL+2, y+5.5);
  pdf.text('CANT.', 138, y+5.5, { align:'center' });
  pdf.text('PRECIO', 158, y+5.5, { align:'right' });
  pdf.text('TOTAL', col2, y+5.5, { align:'right' });
  y += 10;
  ...
  doc.lineas.forEach((l, idx) => {
    ...
    pdf.text(String(l.qty), 138, y+4, { align:'center' });
    pdf.text(fmt(l.price), 158, y+4, { align:'right' });
    pdf.text(fmt(l.qty*l.price), col2, y+4, { align:'right' });
    y += 8;
  ...
  pdf.setFontSize(9);
  pdf.setTextColor(107,114,128);
  pdf.text('Subtotal', boxX+4, y+8);
  pdf.text(`IVA (${doc.iva}%)`, boxX+4, y+16);
  pdf.setFont('helvetica','bold');
  pdf.setFontSize(10);
  pdf.setTextColor(17,24,39);
  ...
  pdf.setFontSize(9);
  pdf.setTextColor(107,114,128);
  pdf.text(fmt(doc.subtotal), col2, y+8, { align:'right' });
  pdf.text(fmt(doc.ivaAmt), col2, y+16, { align:'right' });
  pdf.setFont('helvetica','bold');
  pdf.setFontSize(11);
  pdf.setTextColor(79,70,229);
  pdf.text(fmt(doc.total), col2, y+27, { align:'right' });
  ...
  if(doc.notas) {
    ...
    const lines = pdf.splitTextToSize(doc.notas, W-mL-mR);
    pdf.text(lines, mL, y);
    y += lines.length * 5 + 4;
  }
  ...
  const footerY = 282;
  if(p.iban) pdf.text(`IBAN: ${p.iban}`, mL, footerY);
  if(p.direccion) pdf.text(p.direccion, W/2, footerY, { align:'center' });
  pdf.text(`Generado con Facturas Pro`, col2, footerY, { align:'right' });
...
function submitForm(e) {
  ...
  if(isEdit) {
    ...
    if(doc) {
      ...
      doc.lineas = validLines;
      doc.iva = state.iva;
      doc.subtotal = subtotal;
      doc.ivaAmt = ivaAmt;
      doc.total = total;
      doc.fecha = document.getElementById('f-fecha').value;
      doc.vencimiento = document.getElementById('f-vence').value;
      doc.notas = document.getElementById('f-notas').value.trim();
    }
    saveDocs(docs);
    showToast('Cambios guardados ✓');
    navigate('detail', { id: state.editId });
  } else {
    if(!canCreate()) { showOverlay('mUpgrade'); return; }
    useSlot();
    const doc = {
      ...
      vencimiento: document.getElementById('f-vence').value,
...
function saveSettings() {
  const p = {
    ...
    nif:       document.getElementById('s-nif').value.trim(),
...
function initOnboarding() {
  ...
  document.getElementById('obForm').addEventListener('submit', e => {
    ...
    save(S.PROFILE, {
      ...
      telefono: document.getElementById('ob-tel').value.trim()
    });
    hideOverlay('onboarding');
    document.getElementById('shell').classList.remove('hidden');
    navigate('home');
...
function bindEvents() {
  document.getElementById('backBtn').addEventListener('click', () => {
    ...
    else if(active?.id === 'vForm') {
      state.editId ? navigate('detail', { id: state.editId }) : navigate('home');
    }
    else navigate('home');
  });
  ...
  document.querySelectorAll('.overlay').forEach(ov => {
    ov.addEventListener('click', e => {
      if(e.target === ov && ov.id !== 'onboarding') ov.classList.add('hidden');
    });
  });
}
